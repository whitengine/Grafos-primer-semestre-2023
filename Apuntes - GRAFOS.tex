...
\end{Proposition}
\begin{proof}
Por la observación anterior, $\sum_{v \in V(G)} = 2 \abs{G} \equiv 0 \mod 2$ con lo cual, 
$$
...
\begin{definition}
Sea $G$ un grafo, definimos $G^k$ como la \textbf{potencia} de $G$. Es el grafo que contiene los mismos vértices y las aristas son las originales pero agregando a cada vértice $x$ una arista incidente con cada vértice $y$ a distancia $ d(x,y) \geq k$.
\end{definition}
...
\begin{theorem}[Versión débil]\footnote{La versión fuerte de este teorema, por Alon, Hoory and Linial, 2002, dice que si $d(G) \geq d \neq 2$ y $g(G) \geq g \in \naturals$, entonces $\abs G \geq n_0 (d,g)$.}
Sea $G$ un grafo con $\delta (G) \geq d \geq 2$ y $g(G) \geq g \in \naturals$.
\end{theorem}
\begin{proof}
Sea $v$ un vértice de un ciclo $C$ de largo mínimo, i.e. $\geq g$. Consideremos como $D_i$ al conjunto de vértices a distancia $i$ de $v$ en $G$. Como antes, $\abs {D_0} = 1$, para cada vértice de $D_{i+1}$ tiene un vecino en $D_{i}$ si $i>0$; como cada vértice de $D_i$ tiene un vecino en $D_{i-1}$, se sigue que $\abs{D_{i+1}} \geq (\delta ( G)-1)\abs{D_i} \geq (d-1) \abs{D_i}$ si $i < r$, ya que de lo contrario existiría un ciclo de longitud más chica que $r$.
Reiterando recursivamente esta igualdad, se sigue que $\abs{D_{i+1}} \geq (d-1)^i \abs{D_1}$ para todo $i < r$

Luego como $G = \sqcup_{i} D_i \supset \sqcup_{0\leq i\leq r} D_i$, se sigue que
\begin{enumerate}
\item[Caso $g = 2 r +1$] $\abs G \geq \sum_{0 \leq i \leq r} \abs{D_i} =  1 + \sum{i=0}^{r-1} \abs{D_{i+1}} \geq 1 + \sum_{i=0}^{r-1} (d-1)^i \abs{D_1} \geq 1 + \sum_{i=0}^{r-1} (d-1)^i d$.
...
\end{proposition}
\begin{proof}
Supongamos que no. Es decir, si $C$ es el ciclo de $G$ con menor longitud, se tiene que $\Abs C = g(G) \geq 2 \diam +2$. Es decir, existen dos vértices de $C$, digamos $x,y$ tales que su distancia en $C$ es mayor o igual a $\diam G +1$. En $G$, estos vértices están a distancia menor que $\diam G+1$, sea $P$ el camino mas corto en $G$ que une a $x,y$ (i.e. tiene longitud $< \diam G +1$), luego $P$ no es subgrafo de $G$. Con lo cual, existe un subcamino de $P$ que es un $C$-camino; luego este camino unión el $x-y$ camino más corto de $C$ es un ciclo de longitud más chica que la de $C$, absurdo.
\end{proof}
...
\]
la \textbf{clausura inferior} de $y$, y la \textbf{clausura superior} de $x$, respectivamente. En general, definimos $\ceil X := \bigcup_{x \in X} \ceil x $ y annálogamente $\floor X$, para un conjunto $X \subset V(T)$. Un conjunto $X$ que coincide con $\ceil X$, se dice \textbf{cerrado inferiormente}, análogamente en el otro caso tenemos que es \textbf{cerrado inferiormente}.

Notar que $r$ es el \textit{mínimo} en este orden, y todas las hojas de $T$ son \textit{maximales}. Los extremos de una arista son siempre comparables entre sí, y los elementos de una clausura superior o inferior forman una \textit{cadena} (i.e. son comparables entre sí). Decimos que los vértice a distancia $k$ de $r$ tienen \textbf{altura} $k$ y el conjunto de estos vértices forma el $k$-ésimo \textbf{nivel} de $T$.
...
1\item Supongamos que $T$ genera $G$. Si $S \subset V(T) = V(G)$ y $S$ es inferiormente cerrado, luego las componentes conexas de $G \setminus S$ están generadas por los conjuntos $\floor x$ con $x$ minimal en $T \setminus S$.
\end{enumerate}
\end{lemma}
\begin{proof}
\begin{enumerate}[(i)]
...
\begin{definition}
Llamamos \textbf{subdivisión} de $X$ a cualquier grafo $G$ que se obtiiene de 'subdividir' algunas aristas de $X$ y dibujando encima nuevos vértices. Más precisamente, reemplazamos las aristas de $X$ con nuevos caminos entre sus exxtremos, de manera que estos caminos no se intersecan entre si y tampoco intersecan a $V(X)$ salvo en los extremos.
...
Si un grafo $Y$ contiene un $IX$ como subgrafo, decimos entonces que $X$ es una \textbf{menor} de $Y$, llamamos a $IX$ unu \textbf{modelo} de $X$ en $Y$, y denotamos $X\preccurlyeq Y$
\end{definition}
...
\begin{proposition}
\begin{enumerate}[(i)]\comentario{no entiendo por que vale el segundo item.}
...
\begin{definition}
Un conjunto de $a,B$-caminos es un \textbf{$a,B$-abaníco} si cada par de estos caminos se intersecta SOLO en $a$.
\end{definition}
...
\begin{corollary}
Sean $a$ y $b$ vértices distintos de $G$.
\begin{enumerate}[(i)]
\item Si $ab \not \in E$, entonces el mínimo número de vértices que separan $a$ de $b$ en $G$ es igual al máximo número de $a,b$-caminos INTERNAMENTE disjuntos.
\item El mínimo número de aaritas que separan $a$ de $b$ es igual al máximo número de $a,b$-caminos arista disjuntos.
\end{enumerate}
\end{corollary}
\begin{proof}
\begin{enumerate}[(i)]
