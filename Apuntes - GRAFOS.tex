...
\begin{center}
\includegraphics[width=#1\columnwidth]{"./Figuras/Clase \thenumeroClase /Dibujo \thenumeroDibujo .pdf"}
\captionof{figure}{#2}
\end{center}
}
...
\begin{definition}
Sea $G$ un grafo. Definimos la \textbf{cintura} de $G$ como la mínima longitud de un ciclo en $G$.
\end{definition}
...
\begin{definition}
Los vértices de grado $1$ en un árbol se llaman \textbf{hojas}.
\end{definition}
...
\item $T$ es conexo, pero $T \setminus e$ es disconexo para todo $e \in E(T)$.
\item $T$ es conexo, pero $T \setminus v$ es disconexo para todo $v \in V(T)$ que no sea hoja.
\item $T$ es acíclico, pero $T \cup xy$ tiene un ciclo para cualquier par de vértices $x,y$ no adyacentes.
\end{enumerate}
\end{Theorem}
\begin{proof}
\begin{enumerate}
...
\end{proposition}
\begin{proof}
Tomemos un subgrafo minimalmente conexo $H$, que contenga a todo $V(G)$. Por (iii) del teorema anterior tenemos que $H$ es un árbol.
\end{proof}
...
\begin{Proposition}
Los vértices de un árbol $T$ pueden ser enumerados, digamos $v_1,v_2,\ldots,v_n$ de manera que para todo $i \geq 1$, $v_i$ es hoja en $T[v_1,\ldots,v_i]$ (que es árbol también).
\end{Proposition}
\begin{proof}
