...
\begin{example}
El grafo $K_{2,4}$ no es $2$-lista-coloreable, para eso, basta ver que existe una familia de listas de tamaño $2$ que colorea. Consideremos:

\Inkscape{Coloreo de una elección de listas de tamaño $2$ que es imposible de que funcione.}

Es fácil extender esta idea para cualquier cantidad $k$, utilizando grafos bipartitos completos.
\end{example}


\begin{proposition}
\begin{enumerate}
\item Tenemo que $\ch G \leq \col G$.
\item Hay una versión de Brooks para $\ch G$.
\end{enumerate}
Mäs aún, tenemos el siguiente toerema:
\end{proposition}
