...
\begin{enumerate}[(i)]
\end{enumerate}
\item Tomemos un conjunto de vértices aislados de tamaño máximo $\alpha (G \setminus H)$ en $G \setminus H$, luego como ese conjunto junto con $v \in H$ es aislado en $G$ se sigue que $\alpha (G \setminus H) + 1 \leq \alha (G)$.

\item Como $m(G) \no \leq m(G \setminus H)$, tenemos que $m(G\setminus H) +1 \leq m(G)$. Por otro lado, si agarramos un cubrimiento por subgrafos disjuntos de tipo $\mathcal H$ de $G \setminus H$ y le agregamos $H$, obtenemos un cubrimiento disjunto de $G$, luego $m(G\setminus H) + 1 \geq m(G)$.
\end{proof}
...
\end{exercise}
\begin{solution}
Por el Teorema \ref{th:Nash-williams y Tutte - todo multigrafo con $k$ arboles generadores arista disjuntos si y solo si toda particion tiene almenos k (P-1) elementos}, toda partición $P$ de los vértices de $G$ tiene almenos $2(\abs P -1)$ cross-edges. Luego para todo $x \in G$, considerando la partición $\{ \{x\}, V(G) \setminus \{x\} \}$ vemos que $x$ tiene almenos dos vecinos, i.e. $\delta (G) \geq 2$.

Consideremos el conjunto de subgrafos conexos de $G$ cuyos vértices tienen todos grado par. Y tomemos $H$ máximo con esta propiedad. Debemos ver que existe almenos un grafo $H$ y que el de tamaño máximo genera $G$. Que $G$ tiene un subgrafo conexo de orden par se sigue de que podemos tomar cualquier subciclo; si no tiene subciclos, entonces $G$ es un árbol, luego no puede tener dos árboles arista disjuntos que lo generen (solo hay uno: si mismo).

Supongamos que $H$ no genera $G$, es decir, $H \subsetneq G$. Particionamos $G$ en dos conjuntos de vétices: $V(G) \setminus V(H)$ y $V(H)$. El teorema nos dice que hay almenos $2$ cross-edges. Sea $v \in H$ adyacente a un vértice $x$ fuera de $H$. Notemos que la arista $e = v x$ no está en uno de los dos árboles generadores, pues son arista disjuntos, digamos que no está en $T$, entonces forma un ciclo fundamental $C_e$; así el grafo obtenido por agregar los vértices del ciclo a $H$ es un grafo más grande con grados par. Absurdo, debe ser que $H$ generaba $G$.

Un comentario antes, hay que ser más rigurosos a la hora de ``agregar'' un ciclo a $H$, de manera que todos los vértices sigan teniendo grado par. Sea $C$ un ciclo, si $x$ es un vértce fuera de $H$ en $C$, luego agregamos las aristas de $C$ solamente, y nos queda $x$ de grado $2$. $x$ es un vértice de $H$ entre dos vércies afuera no pasa nada, pues su grado aumenta en $2$. Sin embargo, puede haber problemas si $x,y \in H \cap C$ son adyacentes en $H$, pues no podemos agregar ninguna arista. Sin embargo, podemos quitar la arista $xy$ de $H$ y resolver el problema del grado par: sigue quedando conexo pues $e=xy$ no es el único camino camino que los une en $H$, pues podemos agregar las aristas de un camino en un árbol generador que no contenga a $e$
\end{solution}

Si todos los vértices de $G$ tienen grado par entonces ganamos. Si no, tiene una cantidad impar de vértices de grado impar.

Por inducción en la cantidad de aristas de $G$, podemos asumir que todas las aristas de $G$ están cubiertas por las aristas de $T_1,T_2$ los árboles generadores aristas-disjuntos de $G$. Luego por ele Teorema \ref{Nash-Williams 1964} todo subconjunto no vacío $U$ tiene que $\Abs {G[U]} \leq 2(\abs U -1 )$. En particular podemos tomar $H$ con una catindad máxima de aristas, luego $2 E(H) = \sum_{x \in H} d_H (x)$ implica que $\abs H \delta (H) / 2 \leq \abs{E(H)} \leq \Abs{ G[H]} \leq 2 \abs H - 2$, luego $\delta (H) = 2$.

Si $v \in H$ es un vértice con $\delta (H) = 2$. Podemos hacer inducción si $d_G (v) = d_H (v) = 2$.



Hagámoslo por inducción en el orden $n$ de $G$. Si $n = 1$ no hay nada que probar. En general, tomamos un vértice $v \in G$, debe ser que $d(v) \geq 2$, pues podemos considerar la partición $\{\{v\}, V(G) \setminus \{ v\} \}$ la cual debe tener almenos dos edge-crissings por el Corolario \ref{}. Consideremos entonces el grafo $G[G\setminus \{v\}]$, por el Teorema \ref{th:packing-covering theorem - Bowler - Carmesin - 2015} existe una partición $P$ de este grafo tal que cada $U \in P$ induce un grafo con dos árboles generadores arista-disjuntos y las aristas de $G/P$ (multigrafo) estan cubiertas por $2$ árboles.

Por hipótesis inductiva, los grafos inducidos por $U \in P$ en $G[G\setminus \{v\}]$ tienen un subgrafo conexo generador con todos los grados de sus vértices par. Ahora, dos vértices de $G/P$ están conectados



Ahora, consideremos la partición $P$ de longitud mínima $>1$ de los vértices de $G$, tal que $G[U]$ por cada $U\in P$ está generado por dos árboles disjuntos. Luego por el Corolario \ref{} la cantidad de cross-edges entre particiones es $\geq 2 (\abs P -1)$, con lo cual existen dos $U,U' \in P$ con dos aristas entre ellos. Veamos entonces que $U'' = U \cup U'$ cumple que tiene un subgrafo conexo generador de grados pares, lo cual es imposible pues podríamos cambiar $U,U'$ por $U''$ y obtener una partición más chica, contradiciendo la minimalidad de $P$.

Consideremos el subgrafo inducido $H$ de $G$ generado por dos árboles arista-disjuntos de tamaño máximo tal que $H \subsetneq G$: si $G$ no cumple esta propiedad luego tiene almenos un $H$ de esta forma. Luego $G \setminus H$ también está particionada por grafos generados por dos árboles arista disjuntos. Si $H$ tiene dos aristas en $G \setminus H$ entonces podemos agrandar $H$, y si no podemos achicar la particion de $G \setminus H$. Así podemos asumir que $G \setminus H$ tiene una partición de un solo elento, luego $H$ y $G \setminus H$ tienen dos aristas entre ellos por el Corolario \ref{}, absurdo.

Particionemos a $G$ por una partición de longitud mínima tal que $U \in P$ induce un grafo generado por dos árboles disjuntos y tiene la propiedad de estar generado
...
Haremos inducción en el orden $n$ de $G$. Si $n=1$ no hay nada que probar. En general, si $n > 1$ podemos elegir $v \in G$ y considerar $G' = G \setminus \{v\}$. Tenemos que quizá $G'$ no tiene dos árboles arista disjuntos generadores, pero si existe una partición de vértices $P$ de $G'$ tal que cada grafo inducido $G'[U]$ con $U \in P$ tiene dos árboles arista disjuntos generadores. En efecto, si $T_1,T_2$ son los árboles generadores arista disjuntos de $G$, entonces $T_1$ restringido a $G'$ se despompone en componentes conexas, a su véz, los vértices de estas componentes se pueden



Consideremos un paseo cerrado $W$ en $G$ con la máxima cantidad de vértices (hay almenos un paseo cerrado, pues cualquier ciclo sirve: $G$ debe tener ciclos, de lo contrario es un árbol y por lo tanto no puede tener $2$ árboles generadores arista disjuntos). Si pasa por todos los vértices entonces ganamos porque este es un subgrafo con todos los vértices de grado par. Luego supongamos que no, es decir, podemos particionar los vértices de $G$ en dos: $V(W), V(G \setminus W)$, con lo cual el Corolario \ref{} nos garantiza que hay almenos dos cross-edges.

Sea $x \in W$ adyacente a $z \in G \setminus W$; como $e = xz$ no pertenece al conjunto de aristas de alguno de los dos árboles arista disjuntos que generan $G$, existe un ciclo fundamental $C_e : x z y_1 y_2 \cdots y_r x$ donde todas las aristas (salvo $e$) están, digamos, en $T$. Cambiemos la caminata $W$ por los vértices de $W$ junto con los de $C_e$ y agregamos las aristas de $C_e$ que no estén en $W$. Pero qué pasa si tenemos la secuencia $y_1 x_1 \ldots x_r y_2$ con $y_1,y_2 \not \in W, x_1 ,\ldots, x_r \in W$ (r \geq 2)?
\begin{enumerate}
\item[Caso $y_1 \neq y_2$:]
\item[Caso $y_1 = y_2$:]
\end{enumerate}
Particionando los vértices de $G$ en $W$ y las componentes conexas $C_i$ de $G \setminus W$, y aplicando el Corolario \ref, sabemos que $2r$ cross-edges, i.e. hay almenos dos aristas entre una componente y $W$.
\begin{enumerate}
\item[Caso 1:] Las dos aristas salen de $v \in W$ y tienen extremos $y_1,y_2$. Como $C_i$ es una componente, existe un camino $P$ entre $y_1,y_2$ que es disjunto de $W$. Luego podemos agrandar $W$ haciendo $W y_1 P y_2 W$. Absurdo.
\item[Caso 2:] Las dos aristas salen de vértices distintos, separados por un puente $p$ en $H$. Particionamos entonces los vértices de $H$ en $H_1$ y $H_2$, dados por las componentes de $H - p$. Nuevamente, considerando la partición $H_1,H_2, C_i$ de los vértices de $G$, tenemos que hay almenos dos cross-edges entre algunos de los $H_j$ y $C_i$. Si estamos en el Caso $1$ con $H_j$ en vez de $H$ ganamos. Si no, volvemos a repetir el Caso $2$, ahora con un subgrafo más chico. Eventualmente no podemos particionar mas a $H$ y obtenemos un absurdo.
\end{enumerate}

En conclusión, particionando a $H$ en sus componentes puente $H_1, \ldots, H_s$\footnote{No sé si existe una mejor terminología, pero lo que estamos haciendo es que si $x,y$ son extremos de un puente $p$ en $H$, entonces $H$ ser particiona en dos componentes conexas de $H - xy$; repitendo recursivamente este razonamiento, podemos particionar a $H$ en subgrafos conexos sin puentes, i.e. $2$-arista-conexos.} y a $G \setminus H$ en sus componentes conexas $C_1, \ldots, C_r$. Consideramos la partición de los vértices de $G$ por $\{H_1, \ldots, H_s, C_1, \ldots, C_r\}$, con lo cual el Corolario \ref{} implica que hay almenos $2 (r+ s - 1)$ cross-edges, de los cuales ya sabemos que los únicos entre conjuntos $H_i$ son los $s-1$ puentes, y que entre dos $C_j$ no hay, es decir, los restantes $2r - 1 + s$ cross-edges son entre $H_i$ y $C_j$, luego hay un $H_i =: H'$ con dos cross-edges con un $C_j =: C$. Ahora, pueden ocurrir dos casos:

\begin{enumerate}
\item[Caso 1:] Las dos aristas salen de $v \in H'$ y tienen extremos $y_1,y_2$ en $C$. Como $C$ es conexo, existe un camino $P$ entre $y_1,y_2$ contenido en $C$, en particular disjunto con $H'$. Luego podemos agrandar $H$ agregando $v y_1 P y_2 v$.
\item[Caso 2:] Las dos aristas salen de vértices distintos $v$ y $w$ en $H'$. Tanto en el caso en el que ambos extremos de los cross-edges caigan en vértices distintos de $C$ o iguales, existe un camino entre estos (quizás de longitud $0$), luego agregando este camino junto con los dos cross-edges y eliminandoo las
\end{enumerate}
...
Consideremos el grafo $G'$ dado por los vértices de $G$ unión las aristas de los árboles generadores disjuntos. Basta tomar $H$ como un paseo cerrado por todos los vérticeces de $G'$. Como existe almenos un paseo cerrado en $G'$, podemos tomar uno, digamos $W$, de longitud máxima. Si cubrimos todos los vértices de $G'$ ganamos. Si no, consideremos $G'' = G' - $




Consideremos el $X,Y$-bigrafo $B$ donde $X$ e $Y$ son los vértices de $G$, donde si $x \in X$ e $y \in Y$ tienen una arista si $xy \in E(T_1)$ o si $xy \in E(T_2)$. Cumple la condición de Hall \eqref{eq:condicion de Hall}, pues considerando la partición de $V(G)$ dada por $s_1,\ldots, s_r,V(G) \setminus S (podría ser vacío) con $S = \{s_1,\ldots,s_r\}, el Corolario \ref{} dice que hay suficientes cross-edges, i.e. $\abs S \leq \abs {N(S)}$. Luego por el Teorema de Hall \ref{th:teorema de Hall} existe un matching  que cubre a $X$, el cual es un matching perfecto de $B$ ya que $\abs X = \abs G = \abs Y$.

Con lo cual, consideramos el grafo $H$ generador de $G$ formado por las aristas de matching. Notar que cada vértice tendrá grado $2$, pues si $v \in G$, no existe la arista $vv$ en $B$. Falta ver que es conexo. En efecto, si no lo fuera, particionando $V(G)$ en las componentes conexas de $H$, que por el Corolario \ref{} tiene almenos dos cross-edges entre dos particiones... Creo que la preguntra de ver que $H$ es conexo equivale a ver que los vértices de $B$ se pueden cubrir con ciclos.
