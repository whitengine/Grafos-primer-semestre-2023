...
El problema de encontrar la máxima cantidad de árboles disjuntos en un grafo o la mínima cantidad de árboles que cubren es un problema clásico, el cual resolveremos como corolario de una unificación reciente, obtenida por Bowler y Carmesin: el Teorema de packing-covering.
...
\begin{definition}
Sea $G$ un grafo con una partición de sus vérticces $V(G)$ en $r$ conjuntos, cada uno generando un arbol de $G$. Llamamos \textbf{cross-edge}, a las aristas de $G$ cuyos extremos yacen en distintos conjuntos de la partición.
\end{definition}

\begin{obs}
Sea $G$ un grafo con $k$ árboles arista disjuntos generadores de $G$. Sea $U_1, \ldots, U_r$ una partición de $V(G)$ en $r$ cconjuntos. Entoncces todo árbol generador $T$ de $G$ tiene al menos $r-1$ \textit{cross-edges}, con lo cual, $G$ tiene al menos $k(r-1)$ cross-edges.
\end{obs}
\begin{proof}
...
\end{corollary}
\begin{proof}
Entre cada par de conjuntos $U_i,U_j \in P$ hay al menos $2k$ aristas que conectan a ambos. Con lo cual, $G$ tiene al menos $\binom r 2 2k = r (r-1) k$ cross-edges. Así, se sigue el resultado del teorema anterior.
\end{proof}
...
\begin{definition}
Si $G$ es un grafo plano, llamamos \textbf{caras} a las regiones de $\reals \setminus G$. Al conjunto de caras de $G$ lo denotamos por $\mathcal F (G)$.
\end{definition}
...
\[
    \mathcal F (G) \setminus \{f_1, f_2 \} = \mathcal F (G') \setminus \{f_e\},
\]
...
Recordemos la \textit{subdivisión} de un grafo que aparece al principio de \ref{subsection: subdivisiones}.

\subsubsection{Operaciones que conocemos}

\begin{enumerate}
\item[Subgrafo inducido:] remover vértices.
\item[Subgrafo:] remover vértices y aristas.
\item[Menor:] remover vérties, aristas, y contraer aristas.
\item[Menor topológico:] remover vértices, aristas, y \textbf{contraer vértices} de grado $2$.
\end{enumerate}

Contraer un vértice $w$ de grado $2$ quiere decir que lo quitamos, pero si $u,v$ eran sus vecinos de $w$, entonces nuestro nuevo grafo tiene a $u$ y $v$ como vecinos. Notar que esto es lo mismo que contraer una de las dos aristas que inciden en $w$.
\Dibujo[Ejemplo de contracción de un vértice $w$.]

\subsubsection{Otra perspectiva con respecto a los menores}

\begin{definition}
Sean $G,H$ dos grafos. Decimos que $G$ contiene un \textbf{modelo} de $H$, si $V(G)$ admite una partición $\{v_x | x \in V(H)\}$ en subconjuntos que inducen subgrafos conexos y tal que vértices $x,y \in V(H)$ son adyacentes en $H$ si y solo si $G$ contiene una arista entre $V_x$ y $V_y$.
\end{definition}
...
\subsubsection{Retomando grafos planos}
...
\begin{corollary}
Un grafo plano con $n \geq 3$ vértices tiene a lo más $3n - 6$ aristas. Más aún, toda triangulación con $n$ vértices tiene exactamente $3n-6$ aristas; es decir, un grafo plano es una triangulación si y solo si tiene $3n-6$ aristas.
\end{corollary}
\begin{proof}
Basta demostrar la segunda afirmación gracias a la proposición anterior. En una triangulación, cada cara tiene $3$ aristas en su frontera y cada arista está en la frontera de exactamente dos caras (por el lemma \ref{}). Consideramos ahora el $X,Y$-bigrafo considerando los conjuntos de vértices $X = E(G)$ e $Y = \mathcal F (G)$, donde $ef$ es una arista ($e \in E(G)$, $f \in \mathcal F (G)$) si y solo si $e$ está en la frontera de $f$.

\Dibujo{Ejemplo}

El número de aristas de este grafo bipartito es justamente $2 \abs {E(G)} = 3 \abs{\mathcal F(G)}$. Pero esto nos dice que, usando la fórmula de Euler, i.e. reemplazando $f = \frac 2 3 m$ :
\[
