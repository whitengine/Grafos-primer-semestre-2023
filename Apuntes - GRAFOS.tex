...
\graphicspath{{./Editor de Grafos/Figuras/Clase 13/}} %esto es para que encuentre las figuras hechas con pdf_tex en inkscape
...
\Dibujo{Ver la pagina 41 del Diestel.}
\end{proof}
...
\Dibujo{Ver ilustración de la condición de Tutte.}
...
\Dibujo{Ver la pagina 42 para una ilustración de la contradicción.}
...
Llamamos a un conjunto de vértices $S \subset V$ matcheable con $\mathcal C_{G\setminus S}$ si el grafo bipartito $G_S$\footnote{, $G_S$ es el grafo creado a partir de $G$ por contracción de las componentes $C \in \mathcal_{G\setminus S}$ a vértices y borrando todas las aristas dentro de $S$. Formalmente, $G_S$ es el grafo con conjunto de vértices $S \ccup \mathcal C_{G\setminus S}$ y conjunto de aristas $\set{sC | \text{ existe un vértice $c \in C$ adyacente a $s$ en $G$}}$.} contiene un matching que cubre a $S$.
\end{definition}

\Dibujo{Ilustración de grafo $G_S$.}
...
\Dibujo{Ilustración de la prueba.}
...
\]
porque lluego $G'$ tiene exactamente una cara y una arista menos que $G$. Así obtenemos el resultado por hipótesis inductiva.
\end{proof}
