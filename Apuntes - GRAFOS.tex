...
\begin{theorem}[Menger, 1927]\label{th:teorema de menger}
Sea $G = (V,E)$ un grafo y sean $A,B \subset V$. El mínimo número de vértices que separa a $A$ y $B$ es igual al máximo número de $A,B$-caminos disjuntos.
\end{theorem}
\begin{proof}
...
\Inkscape{Dos dibujos de grafos no topológicamente isomorfos: $H$ tiene una cara con $6$ aristas incidentes, pero $H'$ no.}
...
\Inkscape{Dos dibujos $H$ y $H'$ de un grafo que son combinatorialmente isomorfos, pero no topológicamente.}
...
\Inkscape{Ilustración de $\tilde H$ y $\tilde H'$.}
\end{example}
...
\Inkscape{$\tilde G '$ como dibujo de $G \setminus \{y\}$: el vértice $x$ está representado por un punto $v_{xy}$.}
...
\Inkscape{Dibujo de un grafo con dos vértices de grado $4$ y cuatro vértices de grado $2$.}
...
\Inkscape{Ilustración de la demostración.}
...
\Inkscape{Ilustración de un grafo $H$ que es $\Delta = 4$-coloreable y $v$ afuera de $H$ tiene $4$ vecinos pintados de color: $1,2,3,4$. Si la componente $C_{1,3}$ no fuera un camino, podríamos extender el $\Delta$-coloreo de $H$ a $v$.}
...
\Inkscape{Diagrama de dos caminos $C_{i,j}$ y $C_{i,k}$ que se intersecan en un vértice distinto de $v_i$.}
...
\end{exercise}
\begin{solution}
Procederemos en dos pasos: primero probaremos que sin pérdida de generalidad, podemos suponer que todo vértice de $G$ está contenido en un subgrafo completo de orden $k$ de $G$, luego probaremos que esto implica fácilmente que entre todo par de puntos existen al menos $k-1$ caminos arista-disjuntos, y por lo tanto el ejercicio se segirá de la versión global del Teorema de Menger \ref{th:version global de Menger}.

En efecto, sea $x \in G$ fijo, notemos que tiene $k-1$ vecinos, de lo contrario podríamos extender un $(k-1)$-coloreo de $G \setminus \{x\}$ ($G$ es $k$-cromático crítico) a un $(k-1)$-coloreo de $G$, contradiciendo que $k = \chi (G)$. Más aún, los podemos suponer todos de distinto color (si no podríamos extender un $(k-1)$-coloreo de $G \setminus \{x\}$ a $G$). Ahora, veamos que si $y,z \in N_G(x)$ son tales que $yz \not \in E(G)$ y son de distinto color, entonces $G' := G + yz$ es $k$-cromático crítico y que si $G'$ es $(k-1)$-arista-conexo, entonces $G$ también lo es; esto nos reducirá al caso en el que $G$ cumple la propiedad de que todos sus vértices están en un subgrafo $k$-completo. Agregar una arista entre dos vecinos $y,z$ de distinto color, de $x$ en $G$ no cambia el número cromático, más aún, no cambia que sea $k$-crítico, salvo que exista un vértice $w \neq y,z$ tal que existan $k-1$ cadenas de kempen p

Si $G$ es arista maximal con esta propiedad, entonces
\end{solution}
...
\Inkscape{Arista $e = xy$ con a lo más $\Delta (G)- 1$ vecinos incidentes en cada extremo $x$, $y$.}
\end{obs}
...
\Inkscape{$\Delta$-arista-coloreo de $G -xy$ con $\Delta = 5$.}
...
\Inkscape{Coloreo de una elección de listas de tamaño $2$ que es imposible de que funcione.}
...
Más aún, tenemos el siguiente toerema:

\begin{theorem}[Thomasen (1994)]
...
\Inkscape{Orientación del grafo línea $H$ de $G$.}
...
\Inkscape{Si $G_1$ y $G_2$ son cordales, entonces $G$ también.}
\end{proof}
...
\Inkscape{Expansión de un vértice $x$ en la demostración del Lema \ref{lema:la expansion de un grafo perfecto es perfecto}.}
...
\Inkscape{Ejemplo, $c$ es un coloreo de las aristas tal que $M(x) = \{4\}$. En esta terminología, el color $4$ es \textit{faltante} en $x$.}
...
\Inkscape{$\blue{\alpha}/\red{\beta}$-camino de un arista-coloreo de un grafo $G$.}
...
\Inkscape{Ilustración del coloreo $c_k$ de $G- uv_k$, donde se puede apreciar el $\blue{\alpha}/\red{\beta}$-camino $P$ desde $v_k$.}
...
\Inkscape{``Separamos'' $G$ en dos subgrafos inducidos $G_1$ y $G_2$, tales que no hay vértices de $G_1$ adyacentes a vértices de $G_2$, salvo por $u,v$.}\label{figura:clase 23 - figura 1}
...
\Inkscape{Ilustración del caso (2).}
...
\Inkscape{Ilustración de un $3$-ciclo orientado.}
...
\Inkscape{Ilustración de una orientación de $G =K_5$ con un $4$-camino orientado \blue{$P$} y un subgrafo inducido \red{$T = K_3$}, el cual es un triángulo orientado.}
...
\end{exercise}
\begin{solution}
Sea $G$ un grafo como en el enunciado, y sea $H$ un subgrafo acíclico dirigido maximal (notar que $H$ genera $G$, pues seguro que contiene a un bosque generador). Definamos la siguiente función en los vértices $v$ de $G$:
\[
    f(v) := \begin{cases}
    0 & \text{ si todos los caminos maximales $P$ en $H$ empezando en $v$ tienen longitud par} \\
    1 & \text{ si no.}
    \end{cases}
\]
Afirmamos que $U := f^{-1} (0)$ es un núcleo de $G$.

En efecto, primero veamos que $U$ es un conjunto independiente. Si existieran dos vértices $u,v \in U$ con arista $e = uv \in E(G)$ dirigida hacia $v$, pueden ocurrir dos casos: $e \in H$ y por lo tanto $u$ tiene un camino maximal $P$ en $H$ empezando en $u$ de longitud impar, pues podemos elegir uno de longitud par empezando en $v$ de $H$ y formar otro concatenando con $v$; o si $e \not \in H$, tenemos que por maximalidad de $H$, $H+ e$ tiene un ciclo (par por hipótesis), digamos $C: u v \cdots u$. Con lo cual, este ciclo induce un camino dirigido impar en $H$ saliendo de $v$ hacia $u$; concatenandoló con un camino dirigido par máximal de $H$ saliendo de $u$ obtenemos un camino impar maximal saliendo de $v$, absurdo.

Ahora, veamos que si $s \in G \setminus U$, sale una arista desde $s$ hacia un vértice de $U$. En efecto, sea $P: s u \cdots$ un camino dirigido maximal
\end{solution}
